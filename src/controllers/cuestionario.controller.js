...
const getAll = catchAsync(async (req, res) => {
  const { page = 1, limit = 10, materia_id } = req.query;
...
const create = catchAsync(async (req, res) => {
  ...
  logger.info('Controller: create cuestionario', { usuarioId, title: req.body.title, esIA: req.body.esIA });
  logger.debug(req.body)
  const cuestionario = await cuestionarioService.create(usuarioId, req.body);
  res.status(201).json({ success: true, data: cuestionario });
});
